...
Media provides rich media processing for your Glueful application: image
transformations and on-demand variants, thumbnail generation, and media metadata
extraction (dimensions, duration, codecs). It binds the framework's
`Glueful\Uploader\Contracts\MediaProcessorInterface` seam, so file uploads gain
thumbnails and full metadata and the on-demand image-variant endpoint serves
resized/converted images. It ships the two heavy dependencies it needs —
`intervention/image` and `james-heinrich/getid3`.
