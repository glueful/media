...
namespace Glueful\Extensions\Media\Tests\Unit\Schema;
...
final class SchemaManifestTest extends TestCase
{
    ...
    {
        $base = sys_get_temp_dir() . '/media-mf-' . uniqid('', true);
        ...
        self::assertSame([], $manifest->migrationDescriptors()['glueful/media']);
        self::assertNotContains('glueful/media', $manifest->undeclaredGluefulPackages());
    }

    public function testResolvesWhenEnabledAloneAtItsDeclaredFloor(): void
    {
        $g = $this->manifest();
        $candidates = ['glueful/media' => new \Glueful\Extensions\ExtensionCandidate(
            name: 'glueful/media',
            provider: $g['provider'],
